	...
	public static int getGSOQOS(int index,int value){
		...
		//计算两个萤火虫之间的距离
				public static double returnEDistance(List glowwormId1,List glowwormId2){
					...
					for(int i=0;i<Constants.dimension;i++){
						if(glowwormId1.get(i).toString()==glowwormId2.get(i).toString()){
							cdis.add(0);
						}else{
							cdis.add(1);
					...
					if(cids_count==0){
						...
						return (cids_count-1)/Constants.dimension+1/Constants.dimension*(1-value);
					}
...
//	}
	 ...
	 //初始化萤火虫萤光素
		public static double getLuciferinValue(double F,double l){			
